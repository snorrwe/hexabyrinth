...
class MapSystem final : public ISystem {
    ...
    Quadtree<bool> cells{
        AABB{ MAP_SIZE_X / 2, MAP_SIZE_Y / 2, MAP_SIZE_X, MAP_SIZE_Y / 2 }
    };
...
public:
    explicit MapSystem(Minomaly& engine)
        : engine(engine)
        , hex_component_manager(
              engine.create_component_manager<HexComponent>())
    ...
    void start() override
    {
        ...
        for (auto y = 0; y < MAP_SIZE_Y; ++y) {
            ...
            for (auto x = 0; x < MAP_SIZE_X; ++x) {
                ...
                components.render.dest = { 0, 0, HEX_TEXTURE_SIZE,
                    HEX_TEXTURE_SIZE };
            }
        }
        generate_labyrinth();
...
private:
    void update_cells()
    {
        hex_component_manager->iter(
            [&](auto const& entity, auto const& component) {
